...
const customFieldGid = '1203169756518998'
...
export const batchCreateAsanaEnum: BatchAsanaEnumCreate = async (props) => {
	const queue = props.map((task) => {
		const { fieldGid = customFieldGid, color, name, enabled = true } = task
		return {
			data: { color, name },
			method: 'post',
 ...
 */
export const getAsanaEnum: AsanaGetCustomField = async () => {
	const asana = await asanaClient()
...
type AsanaGetCustomField = () => Promise<AsanaAPI['schemas']['EnumOption'][]>
