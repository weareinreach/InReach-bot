...
export const slackJr = new App({
	...
	logger: {
		debug: (...msgs) => {
			logger.debug('InReachBotJr debug: ' + JSON.stringify(msgs, null, 2))
		},
		info: (...msgs) => {
			logger.info('InReachBotJr info: ' + JSON.stringify(msgs, null, 2))
		},
		warn: (...msgs) => {
			logger.warn('InReachBotJr warn: ' + JSON.stringify(msgs, null, 2))
		},
		error: (...msgs) => {
			logger.error('InReachBotJr error: ' + JSON.stringify(msgs, null, 2))
		},
		setLevel: (level) => {
			level = loglevel
		},
